...
\noindent Portfolio credit risk modeling is not only a risk management tool but also a foundation of modern banking regulation. The Basel II framework introduced the Internal Ratings-Based approach, which links regulatory capital requirements to key credit risk parameters such as probability of default, loss given default, exposure at default, and maturity. Basel III retained the Internal Ratings-Based logic while strengthening the overall capital regime through higher quality capital, capital buffers, and additional constraints designed to improve resilience. \\

\noindent A central feature of the Internal Ratings-Based framework is that it relies on a stylized portfolio model in which default dependence is driven by a common systematic environment. The regulatory approach is designed to be implementable across institutions and sufficiently conservative for solvency purposes. In broad terms, regulatory capital is intended to cover unexpected loss, while expected loss is assumed to be covered through pricing, provisioning, and reserves. \\

\noindent The regulatory framework provides transparency and comparability, but it also has limitations. Its simplifying assumptions are less appropriate for concentrated corporate portfolios, where single-name exposures remain material, and a single systematic driver may not fully capture sectoral concentration or multi-dimensional macroeconomic shocks. Moreover, regulatory inputs and constraints necessarily impose standardization, which can lead to differences between regulatory and internal economic capital. Finally, to the extent that some inputs become more sensitive to the cycle, capital requirements may become procyclical, which is one reason Basel III introduced buffers intended to be built up in good times and used in stress periods.

\section{Practical considerations and model risk}

\noindent Implementing portfolio credit risk models in practice requires data, estimation choices, and governance that can materially influence results. Estimation of probability of default is often challenged by limited default observations, especially for high-grade or low-default portfolios. Estimation of loss given default must address recovery uncertainty and the fact that recoveries may worsen in downturns. Exposure at default is particularly relevant for revolving products and credit lines, where exposures may increase as borrower quality deteriorates. \\

\noindent Another major issue is model risk. Different assumptions about dependence, recovery behavior, and calibration windows can produce substantially different tail loss estimates, even if they agree on average losses. For this reason, robust credit risk management typically combines model-based measurement with scenario analysis, sensitivity checks, benchmarking, and expert judgment rather than relying mechanically on a single model output.
